...
<script type="text/javascript">
    ...
    *   pageNo：当前多少页
    *   pageSize：每一页展示行数
    *   currentSize：当前页返回结果有多少条
    *   idElement：这些值要放到页面哪个位置
    *   callback：回调方法
    *
    * */
    function renderPage(url,total,pageNo,pageSize,currentSize,idElement,callback) {
        var maxPageNo = Math.ceil(total/pageSize);//最大页数
        var paramStartChar = url.indexOf("?") > 0? "& ": "?";//开始的url拼装
        var from = (pageNo - 1) * pageSize +1;//从多少条到多少条
        //在模板中需要替换的对象
        var view = {
        ...
        //点击模板class=page-action的事件
        $(".page-action").click(function(e){
           e.preventDefault();
           //将所点页面位置的pageNo的值设置为data-target的值
